...
function event_combat(e)
	if e.joined then
		...
		if gerton.valid then gerton:CastToNPC():AddToHateList(e.other,1) end
		if davin.valid then davin:CastToNPC():AddToHateList(e.other,1) end
		if lapker.valid then lapker:CastToNPC():AddToHateList(e.other,1) end
		if laima.valid then laima:CastToNPC():AddToHateList(e.other,1) end
		if fomgrut.valid then fomgrut:CastToNPC():AddToHateList(e.other,1) end
		if herga.valid then herga:CastToNPC():AddToHateList(e.other,1) end
	end
end

function event_timer(e)
	if  e.timer == "one" then
		if davin.valid then davin:CastToNPC():Say("Well, we are alive, but I'd hardly say we're well... We have heard of the death of Rodrick, and the Dain is calling on all citizens of Thurgadin to flush out any opposition to the throne. If we're tied to your dealings with the Kromrif we'll be sent to the bottom of the well for sure.") end
		eq.set_timer("two",18 * 1000);
	elseif e.timer == "two" then
		...
		eq.set_timer("three",16 * 1000);
	elseif e.timer == "three" then
		if gerton.valid then gerton:CastToNPC():Say("We all saw what happened when Rodrick agreed to help you. I, for one, am none too eager to take on these new friends of the Dain.") end
		eq.set_timer("four",15 * 1000);
	elseif e.timer == "four" then
		...
		eq.set_timer("five",16 * 1000);
	elseif e.timer == "five" then
		if lapker.valid then lapker:CastToNPC():Say("Go ahead Murdrick, what's this plan of yours?") end
		eq.set_timer("six",12 * 1000);
	...
	elseif e.timer == "seven" then
		...
		eq.set_timer("eight",22 * 1000);
	elseif e.timer == "eight" then
		if davin.valid then davin:CastToNPC():Say("Sounds good boss, but how do you plan on poisoning him?") end
		eq.set_timer("nine",12 * 1000);
	elseif e.timer == "nine" then
		...
		eq.set_timer("ten",22 * 1000);
	elseif e.timer == "ten" then
		if gerton.valid then gerton:CastToNPC():Emote("chuckles, 'Once again, Murdrick, you've put us at ease in the middle of a treacherous predicament. I will volunteer to brew the drink.'") end
		eq.set_timer("eleven",15 * 1000);
