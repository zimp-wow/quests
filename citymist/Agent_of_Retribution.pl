...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("citymist", -763, 27, 4); # pointing out
      $dz->SetSafeReturn("citymist", -763, 27, 4, 138); # right outside kedge facing out... gon' drown some boxes'
      $dz->SetZoneInLocation(-763, 27, 4, 138); # right inside facing in
