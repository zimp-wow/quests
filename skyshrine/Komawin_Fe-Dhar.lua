-- Skyshrine Velious Armor - Monk
local quest_helper		= require('velious_quest_helper');
local SKYSHRINE_ARMOR	= quest_helper.SKYSHRINE_ARMOR;

local QUEST_ITEMS = {
	quest_helper:melee_helmet(SKYSHRINE_ARMOR.Leather_Cap,		31189),	-- Items: White Lotus Headband
	quest_helper:melee_chest(SKYSHRINE_ARMOR.Leather_Tunic,		31190),	-- Items: White Lotus Chest Wraps
	quest_helper:melee_arms(SKYSHRINE_ARMOR.Leather_Sleeves,	31191),	-- Items: White Lotus Arm Wraps
	quest_helper:melee_bracer(SKYSHRINE_ARMOR.Leather_Bracelet,	31192),	-- Items: White Lotus Wrist Wraps
	quest_helper:melee_gloves(SKYSHRINE_ARMOR.Leather_Gloves,	31193),	-- Items: Fist of the White Lotus
	quest_helper:melee_legs(SKYSHRINE_ARMOR.Leather_Leggings,	31194),	-- Items: White Lotus Pants
	quest_helper:melee_boots(SKYSHRINE_ARMOR.Leather_Boots,		31195)	-- Items: White Lotus Slippers
};

function event_say(e)
	if e.other:GetFaction(e.self) == 1 then -- Ally
		if e.message:findi("hail") then
			e.self:Say("Greetings to you. I seek one that has chosen the path of [" .. eq.say_link("i am a monk",false,"monkhood") .. "]. What do you call yourself?");
		elseif e.message:findi("i am a monk") then
