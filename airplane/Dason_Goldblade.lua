function event_say(e)
	...
	elseif e.message:findi("Gregori") then
		...
		e.other:SummonFixedItem(18526); -- Virtuous Knight
	elseif e.message:findi("Dirkog") then
		...
		e.other:SummonFixedItem(18527); -- Walk with Evil
	end
end

function event_trade(e)
	local item_lib = require("items");
	if item_lib.check_turn_in(e.trade, {item1 = 18526}) then --Virtuous Knight
		e.self:Say("Farewell.");
		eq.spawn2(71089,0,0,563,1331.1,-766.9,126.8); -- NPC: Gregori_Lightbringer
		eq.depop_with_timer();
	elseif item_lib.check_turn_in(e.trade, {item1 = 18527}) then --Walk with Evil
		e.self:Say("Farewell.");
