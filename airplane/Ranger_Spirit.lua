function event_say(e)
	...
	elseif e.message:findi("servant of the pine") and e.other:HasClass(Class.RANGER) then
	...
	elseif e.message:findi("servant of the pine") then
		e.self:Say("The trees do not whisper your name.");
	elseif e.message:findi("relinin") and e.other:HasClass(Class.RANGER)  then
		...
		e.other:SummonFixedItem(18546); -- Sisters and Life
	elseif e.message:findi("gordon") and e.other:HasClass(Class.RANGER)  then
		...
		e.other:SummonFixedItem(18547); -- The Mother
	end
end

function event_trade(e)
	local item_lib = require("items");
	if item_lib.check_turn_in(e.trade, {item1 = 18546}) then --Sisters and Life
		e.self:Say("Relinin Skyrunner, I summon thee.");
		eq.spawn2(71095,0,0,617.1,1399.5,-766.9,258.8); -- NPC: Relinin_Skyrunner
		eq.depop_with_timer();
	elseif item_lib.check_turn_in(e.trade, {item1 = 18547}) then --The Mother
