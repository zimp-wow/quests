function event_say(e)
	...
	elseif e.message:findi("great wizard") and e.other:HasClass(Class.WIZARD) then
	...
	elseif e.message:findi("great wizard") then
		e.self:Say("The Arcane eludes you.");
	elseif e.message:findi("Neasin") and e.other:HasClass(Class.WIZARD) then
		...
		e.other:SummonFixedItem(18530); -- Fire and Ice
	elseif e.message:findi("Abec") and e.other:HasClass(Class.WIZARD) then
		...
		e.other:SummonFixedItem(18531); -- Knowledge and Power
	end
end
...
function event_trade(e)
	...
	if item_lib.check_turn_in(e.trade, {item1 = 18530}) then --Fire and Ice
		e.self:Say("Till next time!  Farewell!");
		eq.spawn2(71092,0,0,585,1304.3,-766.9,0.2); -- NPC: Neasin_Leornic
		eq.depop_with_timer();
	elseif item_lib.check_turn_in(e.trade, {item1 = 18531}) then --Knowledge and Power
		e.self:Say("Till next time!  Farewell!");
		eq.spawn2(71079,0,0,585,1304.3,-766.9,0.2); -- NPC: Abec_Ianor
		eq.depop_with_timer();
	end
