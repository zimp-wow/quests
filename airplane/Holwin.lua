function event_say(e)
	...
	elseif e.message:findi("test") and e.other:HasClass(Class.MONK) then
		e.self:Say("Then, choose. Do you wish to be tested by Ton Po or Wu?");
	elseif e.message:findi("test") then
		e.self:Emote("appears to be meditating.");
	elseif e.message:findi("wu") and e.other:HasClass(Class.MONK) then
		e.self:Say("Take this and read it.  Once you are finished, I will summon the Tranquil's most enlightened follower.");
		e.other:SummonFixedItem(18539); -- Strength
	elseif e.message:findi("ton po") and e.other:HasClass(Class.MONK) then
		...
		e.other:SummonFixedItem(18538); -- Tranquility
	end
end

function event_trade(e)
	local item_lib = require("items");
	if item_lib.check_turn_in(e.trade, {item1 = 18539}) then --Strength
		e.self:Say("Farewell.");
		eq.spawn2(71097,0,0,660.0,1332.3,-766.9,379.8); -- NPC: Wu_the_Enlightened
		eq.depop_with_timer();
	elseif item_lib.check_turn_in(e.trade, {item1 = 18538}) then --Tranquility
		e.self:Say("Farewell.");
