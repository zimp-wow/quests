function event_say(e)
	...
	elseif e.message:findi("great rogue") and e.other:HasClass(Class.ROGUE) then
	...
	elseif e.message:findi("great rogue") then
		e.self:Say("Who sent you?");
	elseif e.message:findi("Rayne") and e.other:HasClass(Class.ROGUE) then
		...
		e.other:SummonFixedItem(18522); -- Tome of Shadows
	elseif e.message:findi("Kendrick") and e.other:HasClass(Class.ROGUE) then
		...
		e.other:SummonFixedItem(18523); -- Silent Strider
	end
end

function event_trade(e)
	local item_lib = require("items");
	if item_lib.check_turn_in(e.trade, {item1 = 18522}) then 	--Tome of Shadows
		e.self:Say("Farewell.");
		eq.spawn2(71061,0,0,563.3,1372.6,-766.9,126.8); -- NPC: Rayne
		eq.depop_with_timer();
	elseif item_lib.check_turn_in(e.trade, {item1 = 18523}) then --Silent Strider
		e.self:Say("Farewell.");
