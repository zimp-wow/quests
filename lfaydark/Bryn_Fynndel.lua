...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20636})) then -- Shadowed Code Book
		e.self:Say("Excellent work, " .. e.other:GetName() .. "!  According to these codes, it seems the ghoul boss has been alerted to our probes now that his messenger is missing.  We have a scout, Glidara Myllar, who may be in danger.  Give her this note as soon as you can, she is in grave danger!");
		e.other:QuestReward(e.self,0,0,0,0,20635,1500); -- Warning to Glidara
	elseif(item_lib.check_turn_in(e.trade, {item1 = 20639})) then --Orders for Bryn
