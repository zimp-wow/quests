my $expedition_name = "Crypt of Decay";
my $min_players     = 1;
my $max_players     = 72;
my $dz_zone         = "codecay";
my $dz_version      = 0;
...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("codecay", 0, 0, 0); 
      $dz->SetSafeReturn("codecay", 0, 0, 0, 0);
      $dz->SetZoneInLocation(-179, -56, -93, 130);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("codecay");
  }
}