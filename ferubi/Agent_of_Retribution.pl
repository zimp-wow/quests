...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("ferubi", 1485, 596, 111); # pointing to succor
      $dz->SetSafeReturn("ferubi", 1485, 596, 111, 386);
      $dz->SetZoneInLocation(1485, 596, 111, 386);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("ferubi");
  }
}