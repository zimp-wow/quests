...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      ...
      $dz->SetSafeReturn("gukbottom", 1196.80, -221.69, 78.35); # right outside kedge facing out... gon' drown some boxes'
      $dz->SetZoneInLocation(1196.80, -221.69, 78.35); # right inside facing in
