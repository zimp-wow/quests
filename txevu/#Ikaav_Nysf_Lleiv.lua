...
function event_timer(e)
	if e.timer == "spawn_golem" then
		...
		if stonemasters > 0 then
			...
			if stonemasters == 2 then add_timer = 60000 end
	...
	elseif e.timer == "spawnevent" then
		...
		eq.spawn_condition("txevu",instance_id,4,1)
...
function event_hp(e)
	...
	elseif e.hp_event == 50 then
		...
		if eq.get_entity_list():IsMobSpawnedByNpcTypeID(297088) then
			eq.get_entity_list():MessageClose(e.self,false,120,MT.SayEcho,"The shielding on one of the stonemasters falters as Ikaav Nysf Lliev focuses her energies inward, intensifying her own powers.")
			eq.signal(297088,1) --#Vrex_Stonemaster_Lorasa (297088)
			eq.set_next_hp_event(25)
		end
	elseif e.hp_event == 25 then
		eq.set_timer("curse_of_flames",10000)
		if eq.get_entity_list():IsMobSpawnedByNpcTypeID(297087) then
			eq.get_entity_list():MessageClose(e.self,false,120,MT.SayEcho,"The shielding on the final stonemaster falters as Ikaav Nysf Lliev focuses her energies inward, intensifying her own powers.")
			eq.signal(297087,1) --#Vrex_Stonemaster_Veset (297087)
		end
	end
end
