...
function ChampEvent_Timer(e)
	if e.timer == "spawnevent" then
		...
		local ex, ey, ez = e.self:GetX(), e.self:GetY(), e.self:GetZ();
		...
		eq.unique_spawn(297034, 0, 0, -39, -8, -434, 64); -- NPC: #Mastruq_Champion (297034)
	end
end
...
function ChampEvent_Enter(e)
	if not event_triggered then
		...
		eq.local_emote(
			...
			0,
			200,
			"As you step into the room, all combat stops and every eye turns to you. The champion in the center finishes his opponent and yells 'HOLD!' stopping the Muramites before they can charge."
		);
	end
end
...
function MastruqChampion_Timer(e)
	...
	elseif e.timer == "emote2" then
		...
		eq.local_emote(
			...
			0,
			100,
			"The cry goes up from the crowd, 'KILL THEM!"
		);
	...
	elseif e.timer == "emote5" then
		...
		eq.local_emote(
			...
			0,
			100,
			"A mix of cheers and laughter comes out of the gathering crowd."
		);
	...
	elseif e.timer == "emote8" then
		...
		eq.local_emote(
			...
			0,
			100,
			"The crowd bursts into jeers as a wretched creature pulls itself out of the pond and cowers before the champion."
		);
		eq.unique_spawn(297209, 0, 0, -50, 0, -438, 132); -- NPC: The Runt
...
function TheRunt_Death(e)
	...
	eq.signal(297033, 2);
	eq.signal(297036, 2);
	eq.unique_spawn(297211, 0, 0, -93, 121, -435, 194); -- NPC: #Ixt_Hsek_Syat
