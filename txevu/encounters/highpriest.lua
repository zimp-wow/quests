...
function Trigger_Spawn(e)
	eq.spawn_condition("txevu", instance_id, 1, 1) --enable Vrex_Invoker Spawns
...
function Trigger_Signal(e)
	if e.signal == 1 then --shades reached last waypoint
		...
		if counter == 5 then
			...
			eq.spawn_condition("txevu", instance_id, 1, 0) --disable Vrex_Invoker Spawns
			eq.local_emote(
				{e.self:GetX(), e.self:GetY(), e.self:GetZ()},
				0,
				150,
				"As the final shade lends its voice to the chanting, the prayers reach a crescendo then abruptly fall silent.  A thick mist seeps up from the ground and envelopes the restless wraith.  When the mists fade, an elderly priest stands where the wraith used to be."
			)
...
function Shadeone_Arrive(e)
	if e.wp == 5 then
		...
		e.self:SetGrid(0) --turn off grid so mob stays here indefinitely
	end
end
