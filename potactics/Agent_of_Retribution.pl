my $expedition_name = "Plane of Tactics";
my $min_players     = 1;
my $max_players     = 72;
...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("potactics", 0, 0, 0); 
      $dz->SetSafeReturn("potactics", 0, 0, 0, 0);
      $dz->SetZoneInLocation(-210, 10, -37, 125);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("potactics");
  }
}