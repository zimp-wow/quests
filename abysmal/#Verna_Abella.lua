...
function event_say(e)
	...
	if (e.message:findi("hail")) then
		if (bit.band(greatadventures_turnins, 512) ~= 0) then
...
function event_trade(e)
	...
	-- Trade Logic
	if (item_lib.check_turn_in(e.self,e.trade, {item1 = 67616})) then -- Silius' Writings
		...
		eq.set_data(greatadventures_turnins_bucket, tostring(bit.bor(greatadventures_turnins, 512)));
	end
...
function event_signal(e)
	if(e.signal == 1) then
		...
		eq.signal(279042,22,20 * 1000); -- Galadra Pershan
