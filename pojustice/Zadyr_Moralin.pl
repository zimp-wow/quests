...
sub EVENT_SAY {
  if ($langid == 17) { #Elder Teir'Dal
    if ($text=~/hail/i) {
      quest::say("By the Prince, have you come to [free] me from this place?", 17);
    }
    if ($text=~/free/i) {
      quest::say("I must find a way out of this prison, my trial is approaching, so says the warden. Time is running out for me. There is little chance of reprieve, given my [experiments].", 17);
    }
    ...
    if ($text=~/victories/i) {
      quest::say("Against the traitor-elves, my new-found energies from the life-force of my three slaves' trapped souls provided me with the ability to lay waste to whole legions of our enemies. Yet in their foolishness, my peers of the Prince's Elves sought to stop me, and bring me to [justice].", 17);
    }
    if ($text=~/justice/i) {
