my $expedition_name = "Plane of Torment";
my $min_players     = 1;
my $max_players     = 72;
...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("potorment", 0, 0, 0); 
      $dz->SetSafeReturn("potorment", 0, 0, 0, 0);
      $dz->SetZoneInLocation(-341, 1730, -487, 250);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("potorment");
  }
}