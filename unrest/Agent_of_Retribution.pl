...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("unrest", -.33, -21, 4); # pointing out
      $dz->SetSafeReturn("unrest", -.33, -21, 4, .25); # right outside kedge facing out... gon' drown some boxes'
      $dz->SetZoneInLocation(-.33, -21, 4, .25); # right inside facing in
