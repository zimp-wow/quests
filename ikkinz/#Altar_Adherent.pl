...
sub EVENT_DEATH_COMPLETE {
  ...
  elsif((($class eq "Bard") || ($class eq "Beastlord") || ($class eq "Paladin") || ($class eq "Ranger") || ($class eq "Shadowknight")) && ($qglobals{$instanceid.adhere1} == 2)) {
	quest::ze(0,"The adherent's debris litters the ground. It has been beaten.");
  }
  elsif((($class eq "Berserker") || (plugin::HasClassName($client, "Monk")) || ($class eq "Rogue") || ($class eq "Warrior")) && ($qglobals{$instanceid.adhere1} == 3)) {
	quest::ze(0,"The adherent's debris litters the ground. It has been beaten.");
  }
