...
--#Vrex_Xalkak-s_Sentinel_ (294599)
--[[
1	Warrior
2	Cleric
3	Paladin
...
function event_spawn(e)
	...
	if (qglobals[instance_id .. "vrexkill4"] ==nil) then	
		local random_class=pick_class();
		eq.set_global(instance_id .. "vrexkill4", tostring(random_class), 3, "H6");
	end
end

function event_death_complete(e)
	local qglobals = eq.get_qglobals(e.self);
	if (e.other:HasClass(tonumber(qglobals[instance_id .. "vrexkill4"]))) then
		...
		elseif(qglobals[instance_id .. "vrexkill4"] == "8") then
