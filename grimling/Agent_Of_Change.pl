...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("acrylia", -350, -350, 0); # pointing to guard pineshade
      $dz->SetSafeReturn("acrylia", -50, 50, 0, 150);
      $dz->SetZoneInLocation(-663, 30, 5, 260);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("acrylia");
  }
}