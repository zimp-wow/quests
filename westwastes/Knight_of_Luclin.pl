...
$klakey = $client->AccountID() . "kla";

if ($text=~/hail/i){
  
  if (quest::get_data($klakey) == "") {
    quest::set_data($klakey, 1);
  quest::say("You have defeated the mighty Klandicar! Soon the Scions will grant you passage... Then Aten Ha Ra can finally be put to rest.");
