...
function event_trade(e)
    ...
    if item_lib.check_turn_in(e.trade, {item1 = 2387}) then -- Item: Interrogators Badge
    ...
    elseif tonumber(qglobals.qeynos_badge3) == 1 and item_lib.check_turn_in(e.trade, {item1 = 18295}) then -- starts event when Research Briefing handed in
	...
	elseif jars == 3 and not eq.get_entity_list():IsMobSpawnedByNpcTypeID(2179) and tonumber(qglobals.qeynos_badge3) == 2 and (item_lib.check_turn_in(e.trade, {item1 = 2587, item2 = 2588 , item3 = 2589})) then -- many checks to confirm event is a sucess!
