...
function event_reset()
	eq.depop_all(14147); 	-- NPC: #Morley_Murrain
	eq.depop_all(14127); 	-- NPC: #Markus_Cachexia
	eq.depop_all(14140); 	-- NPC: skeletal_servant
	eq.depop_all(14148); 	-- NPC: Ghoul
	eq.depop_all(14146); 	-- NPC: #Theodore_Exanthem
end
...
function event_trade(e)
	...
	if item_lib.check_turn_in(e.trade, {item1 = 18292 }) then -- Item: Interrogator's Briefing
		...
		eq.set_timer("brief", 10 * 1000);
		name = e.other:GetName();
		brief = 0;
		eq.set_global("qeynos_badge2","2",5,"F");		-- QGlobal: Badge Globals
		e.other:QuestReward(e.self,0,0,0,0,2344,1000);	-- Item: Confession Document
	elseif item_lib.check_turn_in(e.trade, {item1 = 2390,item2 = 2391,item3 = 2395}) then -- Items: Head of Markus Cachexia, Head of Morley Murrain, Theodore's Confession
		...
		e.other:SummonItem(2387);					-- Item: Interrogators Badge
		eq.set_global("qeynos_badge2","6",5,"F");	-- QGlobal: Badge Globals
	end
...
function event_waypoint_arrive(e)
	...
	elseif e.wp == 3 then
		...
		eq.set_global("qeynos_badge2","4",5,"F");		-- QGlobal: Badge Globals
	end
end
...
function event_signal(e)
	...
	elseif e.signal == 3 then
		...
		e.self:DoAnim(8);
		eq.signal(14146,3,300); -- NPC: #Theodore_Exanthem
	...
	elseif e.signal == 5 then
		...
		eq.signal(14146,5,2000); -- NPC: #Theodore_Exanthem
	elseif e.signal == 6 then
		e.self:Say("[Take us to the location] of Markus Cachexia and Morley Murrain!");
		eq.signal(14146,6,1); -- NPC: #Theodore_Exanthem
