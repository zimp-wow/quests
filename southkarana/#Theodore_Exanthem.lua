...
function event_signal(e)
	if e.signal == 1 then
		...
		eq.signal(14050,2,5000); -- NPC: an_interrogator
	elseif e.signal == 2 then
		...
		eq.signal(14050,3,5000); -- NPC: an_interrogator
	elseif e.signal == 3 then
		e.self:Emote("reels back, his eyes wide with shock. 'Ow,' he says nervously.");
		e.self:DoAnim(14);
		eq.signal(14050,4,5000); -- NPC: an_interrogator
	elseif e.signal == 4 then
		...
		eq.signal(14050,5,5000); -- NPC: an_interrogator
	...
	elseif e.signal == 7 then
		...
		eq.signal(14050,8,5000); -- NPC: an_interrogator
	end
end

function event_combat(e)
	if e.joined then
		eq.set_timer("defeat", 3 * 1000);
	end
end
...
function event_trade(e)
	...
	if tonumber(qglobals.qeynos_badge2) >= 2 and item_lib.check_turn_in(e.trade, {item1 = 2344}) then -- Item: Confession Document
		...
		e.other:SummonItem(2395);					-- Item: Theodore's Confession
        eq.set_global("qeynos_badge2","5",5,"F");	-- QGlobal: Badge Globals
