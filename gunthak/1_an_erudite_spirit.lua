...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 59030})) then -- Alina's emblem
		...
		e.other:QuestReward(e.self,0,0,0,0,59011,eq.ExpHelper(45, 5)); -- Spell: Temperance, 5% of lvl 45 exp
...
function reset_spawn()
	...
	local spawnpoint = eq.get_entity_list():GetSpawnByID(46892);
	spawnpoint:Enable();
	spawnpoint:Reset();
end
