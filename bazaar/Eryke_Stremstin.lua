...
local qglobals = eq.get_qglobals(e.other);
	...
	elseif(qglobals["monk_epic"] >= "6" and e.message:findi("price")) then
		e.self:Say("I would be willing to part with the information and items for a meager amount of 1000 platinum pieces.");
	elseif(qglobals["monk_epic"] >= "6" and e.message:findi("polished")) then
...
function event_trade(e)
	...
	elseif(qglobals["monk_epic"] >= "6" and item_lib.check_turn_in(e.trade, {platinum = 1000})) then
