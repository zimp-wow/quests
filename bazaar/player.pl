...
sub EVENT_TIMER {
    ...
    use constant {
        ...
        WARNING_START_MINUTES    => 4,
        WARNING_INTERVAL_SECONDS => 60,
    };

    my $shadow_bazaar = quest::get_data("shadow_bazaar_instanceid");

    if (!$shadow_bazaar) {
        quest::debug("Creating Shadow Bazaar");
        $shadow_bazaar = quest::CreateInstance('bazaar', 0, 604800);
        quest::set_data("shadow_bazaar_instanceid", $shadow_bazaar, 604800);
    }

    $client->AssignToInstance($shadow_bazaar);
    ...
    if ($timer eq "check_idle") { 
        ...
        if ($instanceid == $shadow_bazaar) { # inside private instance   
            if ($idle and $idle_track % (IDLE_THRESHOLD_SECONDS) == 0) {
                $client->Message(15, "You are in the Shadow Bazaar. You will be transported to the Bazaar once you are no longer idle.");
            }
            elsif (!$idle) {
                #if ($non_idle_c < 3) {
                #    $client->SetEntityVariable("non_idle_count", $non_idle_c + 1);
                #} else {
                    $client->Message(15, "You are no longer idle. Returning you to the Bazaar.");
            ...
            elsif ($idle and $idle_track >= (IDLE_THRESHOLD_SECONDS)) {
                $client->Message(15, "You have been idle for " . (IDLE_THRESHOLD_SECONDS / 60) . " minutes. Transporting you to the Shadow Bazaar...");
                $client->MovePCInstance(151, $shadow_bazaar, $client->GetX(), $client->GetY(), $client->GetZ(), $client->GetHeading());
            }
            elsif ($idle) {
                my $minutes_idle = int($idle_track / 60);
                if ($idle_track % WARNING_INTERVAL_SECONDS == 0 and $minutes_idle >= WARNING_START_MINUTES) {
                    ...
                    $client->Message(15, "You have been idle for $minutes_idle_word minutes. You will be transported to the Shadow Bazaar soon.");
                }
            }
        }        
    }
