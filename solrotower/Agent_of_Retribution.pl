my $expedition_name = "Sol Ro Tower";
my $min_players     = 1;
my $max_players     = 72;
...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("solrotower", 0, 0, 0); 
      $dz->SetSafeReturn("solrotower", 0, 0, 0, 0);
      $dz->SetZoneInLocation(-1, -2915, -763, 0);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("solrotower");
  }
}