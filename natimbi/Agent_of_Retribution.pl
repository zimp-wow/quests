...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("natimbi", -1557, -853, 239); # pointing to guard pineshade
      $dz->SetSafeReturn("natimbi", -1557, -853, 239, 5);
      $dz->SetZoneInLocation(-1557, -853, 239, 5);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("natimbi");
  }
}