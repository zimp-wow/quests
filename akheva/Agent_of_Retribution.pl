my $expedition_name = "Akheva Ruins";
my $min_players     = 1;
my $max_players     = 72;
my $dz_zone         = "akheva";
my $dz_version      = 0;
...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("akheva", -350, -350, 0); # pointing to guard pineshade
      $dz->SetSafeReturn("akheva", -50, 50, 0, 150);
      $dz->SetZoneInLocation(63, -1401, 22, 0);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("akheva");
  }
}