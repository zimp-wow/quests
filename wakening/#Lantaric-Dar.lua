    ...
    if not epic_disabled and e.other:HasClass(Class.BARD) then
        ...
        if e.message:findi("hail") then
            if qglobals["bard15"] == "1" then
                e.self:Say("Journey to the necropolis and stop the mercenaries " .. e.other:GetRaceName() .. ".  May Veeshan protect you.")
            ...
            elseif qglobals["bard15"] == "3" then
                e.self:Say("Return to the necropolis and stop Vesthon before it is too late " .. e.other:GetRaceName() .. "!")
            ...
            elseif e.other:HasItem(20542) then
                e.self:Emote("seems surprised as you approach him.  He peers down at you with an inquisitive glint in his eyes. 'I sense a familiar aura around you " .. e.other:GetRaceName() .. "  One I have not felt in a long time.  Tell me " .. e.other:GetRaceName() .. ", do you have news of a man named Baldric?")			
            else
                -- 1.5 start
        ...
        elseif e.message:findi("Baldric Slezaf") then -- you don't say this for prequest, prequest should just set bard15 to 1. This lore is covered by the prequest
            if e.other:HasItem(20542) then
                e.self:Emote("'s eyes glitter with energy as you mention Baldric's Name. 'So I was not mistaken, you do know Baldric.  He was a friend, and as a great a mortal as I have ever known.  You must be an exceptional mortal yourself if Baldric saw fit to bequeath the blade to you.  Yes I know of it.  I could feel it's energy around your body.  Perhaps fate has brought you to me this day.  I am in great need of help " .. e.other:GetRaceName() .. ".")
