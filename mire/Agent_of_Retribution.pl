my $expedition_name = "The Frozen Catacombs of Miragul";
my $min_players     = 1;
my $max_players     = 72;
my $dz_zone         = "mire";
my $dz_version      = 0;
...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("mire", 238.0, 987.0, -24.90); # pointing to guard pineshade
      $dz->SetSafeReturn("tox", 0,0,0,0);
      $dz->SetZoneInLocation(1103, 610, 0, -200); # bridge in crushbone
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("mire");
  }
}