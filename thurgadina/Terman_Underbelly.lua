-- Thurg Velious Armor - Shaman
local quest_helper	= require('velious_quest_helper');
local THURG_ARMOR	= quest_helper.THURG_ARMOR;

local QUEST_ITEMS = {
	quest_helper:priest_helmet(THURG_ARMOR.Chain_Coif,		31007),	-- Items: Rune Crafter's Helm
	quest_helper:priest_chest(THURG_ARMOR.Chain_Tunic,		31008),	-- Items: Rune Crafter's Breastplate
	quest_helper:priest_arms(THURG_ARMOR.Chain_Sleeves,		31009),	-- Items: Rune Crafter's Vambraces
	quest_helper:priest_bracer(THURG_ARMOR.Chain_Bracer,	31010),	-- Items: Rune Crafter's Bracer
	quest_helper:priest_gloves(THURG_ARMOR.Chain_Gauntlets,	31011),	-- Items: Rune Crafter's Gauntlets
	quest_helper:priest_legs(THURG_ARMOR.Chain_Leggings,	31012),	-- Items: Rune Crafter's Greaves
	quest_helper:priest_boots(THURG_ARMOR.Chain_Boots,		31013)	-- Items: Rune Crafter's Boots
};
...
function event_say(e)
	if e.other:GetFaction(e.self) <= 3  then -- Warmly or better
		if e.message:findi("hail") then
			...
			e.self:Say("Greetings. I only wish to speak to a shaman. Are you a ["..eq.say_link("I am a shaman",false,"shaman").."]?");
		elseif e.message:findi("i am a shaman") then
