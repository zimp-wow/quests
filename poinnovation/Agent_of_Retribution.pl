my $expedition_name = "Plane of Innovation";
my $min_players     = 1;
my $max_players     = 72;
...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("poinnovation", 0, 0, 0); 
      $dz->SetSafeReturn("poinnovation", 0, 0, 0, 0);
      $dz->SetZoneInLocation(241, 550, -49, 200);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("poinnovation");
  }
}