my $expedition_name = "Emerald Jungle";
my $min_players     = 1;
my $max_players     = 72;
...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("emeraldjungle", 238.0, 987.0, -24.90); # 
      $dz->SetSafeReturn("emeraldjungle", -7025, 2021, -60, 484.0);
      $dz->SetZoneInLocation(1525, -3375, -340, 40); # Ent
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("emeraldjungle");
  }
}