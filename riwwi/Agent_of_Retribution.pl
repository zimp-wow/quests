...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("riwwi", 454, -650, 35); # pointing to guard pineshade
      $dz->SetSafeReturn("riwwi", 454, -650, 35, 421);
      $dz->SetZoneInLocation(454, -650, 35, 421);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("riwwi");
  }
}