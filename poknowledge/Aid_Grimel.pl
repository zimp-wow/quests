...
sub EVENT_SAY {
  if($text=~/hail/i) {
    ...
    if(defined($qglobals{pop_cod_final}) && ($qglobals{pop_cod_final} == 1)) { #Must have killed Bertox and rescued Mylik and Adler
      if($text=~/willing to help and am a master of trades/i) {
      ...
      if($text=~/versed in the art of smithing/i) {
        if($client->GetRawSkill(63) >= 220) { #Blacksmithing
      ...
      if(($text=~/put my jewel craft skills to the test/i) && plugin::check_hasitem($client, 16250)) { #Clay Signet
        if($client->GetRawSkill(68) >= 220) { #Jewelcraft
      ...
      if(($text=~/ready to use some clay/i) && plugin::check_hasitem($client, 16251)) { #Wooden Signet
        if($client->GetRawSkill(69) >= 220) { #Pottery
      ...
      if(($text=~/am skilled with the needle/i) && plugin::check_hasitem($client, 16252)) { #Metal Signet
        if($client->GetRawSkill(61) >= 220) { #Tailoring
      ...
      if(($text=~/master chef/i) && plugin::check_hasitem($client, 16254)) { #Runed Signet
        if($client->GetRawSkill(60) >= 220) { #Baking
