...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("hole", -1054, 620, -78); # pointing to guard pineshade
      $dz->SetSafeReturn("hole", -1054, 620, -78, 184);
      $dz->SetZoneInLocation(-1054, 620, -78, 184);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("hole");
  }
}