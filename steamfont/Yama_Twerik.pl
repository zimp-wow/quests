...
my $slave = 56190; #Inactive_Clockwork (steamfont)
my $normal_grid = 180; #Normal pathing grid (steamfont)
my $event_grid = 181; #Event pathing grid (steamfont)
...
sub EVENT_WAYPOINT_ARRIVE {
  if (($event == 1) && ($wp == 1)) {
    quest::stop();
    quest::moveto(-1581, 1285, -108, 114, 1); #Clockwork location (steamfont)
