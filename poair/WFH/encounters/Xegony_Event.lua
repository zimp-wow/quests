...
function XegonyHP(e)
	...
	elseif e.hp_event == 25 then
		eq.set_next_hp_event(13);
		CallGuards();
	elseif e.hp_event == 13 then
		if named_dead ~= 6 then
			e.self:GotoBind()
			e.self:SetHP(e.self:GetMaxHP() * 0.25); -- Standard Gate heal
...
function EventTimers(e)
	...
	elseif e.timer == "memblur" then		
		if e.self:IsEngaged() then
			if e.self:GetNPCTypeID() ~= 215056 then -- If not Xegony
				if math.random(100) <= 30 then --30% memblur chance
					e.self:WipeHateList()
				end
			end
...
function EventDeathCounter(e)
	named_dead = named_dead + 1;
end

function EventWin(e)
	eq.stop_all_timers();
	eq.spawn2(215412,0,0,e.self:GetX(), e.self:GetY(), e.self:GetZ()-20, e.self:GetHeading());	--#Essence_of_Air (215412)
	eq.signal(215438 , 215056); -- NPC: zone_status npc, originating npc
...
function event_encounter_load(e)
	...
	--Boss Signals
	eq.register_npc_event("Xegony_Event", Event.death_complete, 215416, EventDeathCounter);	--#Nuquernal_Belegrodian (215416)
	eq.register_npc_event("Xegony_Event", Event.death_complete, 215414, EventDeathCounter);	--#Huridaf_Vorgaasna (215414)
	eq.register_npc_event("Xegony_Event", Event.death_complete, 215421, EventDeathCounter);	--#Weruis_Herfadban (215421)
	eq.register_npc_event("Xegony_Event", Event.death_complete, 215399, EventDeathCounter);	--#an_elemental_arbitor (215399)
	eq.register_npc_event("Xegony_Event", Event.death_complete, 215422, EventDeathCounter);	--#Wesreh_Galleantan (215422)
	eq.register_npc_event("Xegony_Event", Event.death_complete, 215418, EventDeathCounter);	--#Rindaler_Egulrtan (215418)
