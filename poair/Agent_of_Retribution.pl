my $expedition_name = "Plane of Air";
my $min_players     = 1;
my $max_players     = 72;
my $dz_zone         = "poair";
my $dz_version      = 0;
...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("poair", 0, 0, 0); 
      $dz->SetSafeReturn("poair", 0, 0, 0, 0);
      $dz->SetZoneInLocation(543, 890, -90, -240);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("poair");
  }
}