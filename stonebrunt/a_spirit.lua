function event_say(e)
    if(e.message:findi("Hail")) then
        e.self:Emote("flares brightly as you attempt to speak with it. By listening very closely you're brely able to make out it's frantic whispering. It keep refering to one of the Kejekans, one that apparently slew him as he attempted to cleanse the land of them. It wants nothing more than to destroy who killed him, but it cannot in this [" .. eq.say_link("form") .. "].");
    elseif(e.message:findi("form")) then
        e.self:Emote("flares brightly again as it explains there isn't enough time for it to explain. It then begins to whisper again, telling you that it commands you the aid it in it's [" .. eq.say_link("revenge") .. "]. You make out, through the barely audible whispering, that if you do help it, you will be rewarded.");
    elseif(e.message:findi("revenge")) then
        e.self:Emote("whispers that he will need to become whole again before he can defeat the fool animal that slew him. The spirit admits to one mistake before coming here to wipe the Kejekans. In an earlier [" .. eq.say_link("campaign") .. "], he lost a very powerful weapon. That weapon would enable him to take on a form likened to his former self. If you return this weapon to the spirit, and then help him to defeat his enemy, the reward will be yours.");
...
function event_trade(e)
    ...
    if(item_lib.check_turn_in(e.trade, {item1 = 2100})) then -- Grimy Lance
        ...
        eq.unique_spawn(100221, 323, 0, -70, 474, 509, 67); -- Ridossan_the_Unliving
        eq.depop_withtimer();
    end
