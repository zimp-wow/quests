...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("growthplane", 3016, -2522, -22); # AOR at zone in
      $dz->SetSafeReturn("wakening", -1120, 511, -167, 60); # safe outside building in wakening
      $dz->SetZoneInLocation(3016, -2522, -22, 484); # At AOR
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("growthplane");
  }
}