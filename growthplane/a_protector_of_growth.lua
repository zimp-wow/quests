...
function AggroCheck(e)
	...
	if client_list ~= nil then
		for client in client_list.entries do 
			if not client:IsInvisible(e.self) and not client:GetFeigned() and not client:GetGM() and client:GetModCharacterFactionLevel(226) <= -701   then	--Clerics of Tunare(Primary PoG Faction)
