function event_say(e)
	...
	elseif e.message:findi("third part") then
	    ...
	    e.other:SummonItem(17053); -- Item: Map Binding
	end
end

function event_trade(e)
	local item_lib = require("items");
	if item_lib.check_turn_in(e.trade, {item1 = 30046}) then -- Item: Pieces of the Iceclad Map
		...
		eq.get_entity_list():MessageClose(e.self, true, 100, MT.SayEcho, "As he starts to try and crawl up the hill, a sleek figure darts towards him.")
		e.other:QuestReward(e.self,{exp = 50000});
		e.other:SummonItem(30047); -- Item: Completed Map of Iceclad
		eq.spawn2(116008,0,0,1002,6354,-32.09,130):AddToHateList(e.other,1); -- NPC: a_snow_cougar
	end
