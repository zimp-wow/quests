function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings to you " .. e.other:GetName() .. ". I welcome you to the Scouts of Tunare training halls. I have seen many scouts pass through my halls with unimaginable abilities and daggers guided by Tunare herself. I cannot say for certain what the future has in store for you. But I can say that wherever your adventures may take you it is always wise to have the proper [" .. eq.say_link("training") .. "].");
	elseif(e.message:findi("training")) then
		e.self:Say("The training that I speak of is only intended for rogues of Tunare that wish to develop skills that will mold them into the most deadly assassins in all of the lands. If you are a [" .. eq.say_link("I am a rogue of tunare",false,"rogue of Tunare") .. "] then I will be happy to explain the training exercises to you. However. if you are not a rogue I suggest you seek training from another house.");
	elseif(e.message:findi("i am a rogue of tunare")) then
		e.self:Say("Excellent " .. e.other:GetName() .. ". let us get started then. Your training will test your fighting and gathering skills respectively. because of the items you will be required to gather all over the Faydark for [" .. eq.say_link("magical armor materials") .. "].");
	elseif(e.message:findi("magical armor materials")) then
		e.self:Say("First things first. here is your Assassins Mail Assembly Kit. Inside this kit you will combine different magical items. which in certain quantities in your mail kit can create armor materials for specific pieces of Tunarean Scouts Armor. Once you have completed a specific armor material you can then place it in a forge with the applicable pattern to create your piece of armor.[" .. eq.say_link("continue") .. "]");
		e.other:SummonItem(17365)
	elseif(e.message:findi("continue")) then
		e.self:Say("Simply tell me what armor piece you want to craft and I can provide you with the material recipes and armor patterns for [" .. eq.say_link("Coifs") .. "], [" .. eq.say_link("Bracers") .. "], [" .. eq.say_link("Sleeves") .. "], [" .. eq.say_link("Boots") .. "], [" .. eq.say_link("Legplates") .. "], [" .. eq.say_link("Gauntlets") .. "] and [" .. eq.say_link("Tunics") .. "].");
		e.other:SummonItem(17365)
	...
	elseif(e.message:findi("favor")) then
		e.self:Say("I have recently been trying my hand at weaponcrafting and I can say that it is going quite well. Before I can practice my skills however I need some [" .. eq.say_link("specific items") .. "] that will enable me to begin crafting daggers for our young scouts like yourself that have completed the training exercises.");
