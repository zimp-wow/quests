function event_say(e)
	...
	elseif e.message:findi("contributed") then
		if e.other:HasClass(Class.ROGUE) then
			...
			e.other:SummonItem(12184); -- Item: Useless Token
		else
...
function event_trade(e)
	...
	if item_lib.check_turn_in(e.trade, {item1 = 12183}) then -- Item: Crushbone Information
		...
		e.other:SummonItem(2914);	-- Item: Scouts Cape
		e.other:Faction(316,10); -- Faction: Tunare's Scouts
	end
