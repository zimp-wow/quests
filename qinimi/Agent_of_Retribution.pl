...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("qinimi", -1480, -298, -15); # AoR
      $dz->SetSafeReturn("qinimi", -1480, -298, -15, 37);
      $dz->SetZoneInLocation(-1480, -298, -15, 37);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("qinimi");
  }
}