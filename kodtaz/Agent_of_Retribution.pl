...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
    ...
    if ($dz) {
      $dz->SetCompass("kodtaz", -1368, 1566, -300); # pointing to guard pineshade
      $dz->SetSafeReturn("kodtaz", -1368, 1566, -300, 257);
      $dz->SetZoneInLocation(-1368, 1566, -300, 257);
      $dz->AddReplayLockout(79200); # immediately add a 22 hour replay lockout on creation
      quest::say("Tell me when you're [" . quest::saylink("ready") . "] to enter");
    }
  }
  elsif ($text =~ /ready/i) {
    $client->MovePCDynamicZone("kodtaz");
  }
}