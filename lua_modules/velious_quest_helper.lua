local helper				= {}
local multiquest_enabled	= false; -- Set to true to enable multiquesting on all velious armors. Requires PR# 4593 minimum
...
function helper:quest_turn_in(event, faction_req, items, callback)
	...
	if multiquest_enabled then
		event.self:MultiQuestEnable();
	end

	if faction <= faction_req then
		for i, quest in ipairs(items) do
			if item_lib.check_turn_in(event.trade, quest.turn_in) then
				event.other:SummonItem(quest.reward);
				event.other:QuestReward(event.self,{exp = 1000});
				callback(self, event);
				return;
			end
		end
