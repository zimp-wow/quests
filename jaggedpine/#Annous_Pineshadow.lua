    ...
    if(fac == 1) then
        ...
        elseif(tonumber(qglobals.galeweaver) == 1) then
            if(e.message:findi("prove myself worthy")) then
                e.self:Say("Then your endeavor shall continue, my child. Take this scroll -- to you now it holds no meaning but in due time you shall understand. Keep the scroll close to you until you have sought and successfully acquired the [" .. eq.say_link("Dormant Crystal Sphere") .. "] and [" .. eq.say_link("The Rainkeeper's Writ") .. "].");
                e.other:SummonItem(8962); -- Ancient White Scroll
            elseif(e.message:findi("dormant crystal sphere")) then
                e.self:Say("There are several magical spheres throughout Norrath but this sphere is [" .. eq.say_link("unique to The Rainkeeper") .. "]. It is said that when Karana blessed this area with his first storm in acceptance of the nomadic tribe that would settle here, that the droplets of rain left upon the pines and blades of grass was gathered by one of these nomads and stored in a plain clay jar that was then buried as a symbol of their own gratitude toward the storm. Nearly a century later when Karana's Shrine and the Throne of Karana's Thunder were near completion, one known as Tanirun Galeweaver found the jar. The water was still inside, preserved by means unexplainable through simple reason. It is said that Tanirun poured the water into his own hands and wove a spell, sealing the wind within the orb of this ancient rain that was now crystallized by Tanirun's spell.");
...
function event_trade(e)
	...
	if(fac == 1) then
		if(item_lib.check_turn_in(e.trade, {item1 = 8956})) then -- Azure Idol of Karana
		...
		elseif(item_lib.check_turn_in(e.trade, {item1 = 8964,item2 = 8963,item3 = 8962}) and tonumber(qglobals.galeweaver) == 1) then -- The Rainkeepers Writ, Dormant Crystal Sphere & Ancient White Scroll
		...
		elseif(item_lib.check_turn_in(e.trade, {item1 = 8966}) and tonumber(qglobals.galeweaver) == 2) then -- Orb of the Gale
