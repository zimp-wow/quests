...
function event_trade(e)
	...
	if fac <= 4 then
		if tonumber(qglobals.qeynos_badge5) == 1 and item_lib.check_turn_in(e.trade, {item1 = 8283}) then -- Item: Official Warning
			...
			eq.set_global("qeynos_badge5","2",5,"F");	-- QGlobal: Badge Gloebals
		elseif tonumber(qglobals.qeynos_badge5) == 2 and item_lib.check_turn_in(e.trade, {item1 = 8199}) then -- Item: Dryad Pate
			...
			e.other:SummonItem(8284);	-- Item: Finewine Family Brandy
			e.other:SummonItem(8283);	-- Item: Official Warning
		elseif tonumber(qglobals.qeynos_badge5) == 2 and item_lib.check_turn_in(e.trade, {item1 = 8198}) then -- Item: Potameid Pate
			e.self:Say("Hmm. . . This is interesting. . . The tast is very refined but sort of. . . odd. . . We're almost there but I don't think you have the recipe quite right. Rather than that peassant Brandy, use this in your recipe instead. And rather than whatever other 'meat' you were using before use a Panther Liver. You do seem to have some skill as a chef, mayhaps you could work for me some day. Prepare some Panther Pate for me. Give me the Panther Pate and let me read that warning again.");
			e.other:QuestReward(e.self,{exp = 1000});
			e.other:SummonItem(8284);	-- Item: Finewine Family Brandy
			e.other:SummonItem(8283);	-- Item: Official Warning
		elseif tonumber(qglobals.qeynos_badge5) == 2 and item_lib.check_turn_in(e.trade, {item1 = 8278, item2 = 8283}) then -- Items: Panther Pate and Official Warning
			...
			e.other:SummonItem(8279);	-- Item: Stack of Shift Reports
		end
	end
